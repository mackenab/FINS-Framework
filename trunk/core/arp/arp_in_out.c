...
void arp_in(struct finsFrame *fins_received) {
	...
	if (fins_arp_in->dataOrCtrl == DATA && (fins_arp_in->destinationID.id == (unsigned char) ARPID)) {
		PRINT_DEBUG("ARP Data, ff=%p", fins_received);
		...
		PRINT_DEBUG("ARP Control, ff=%p", fins_received);
...
void output_arp_queue(struct finsFrame *fins_arp_out) {
	if (fins_arp_out->dataOrCtrl == CONTROL) {
		PRINT_DEBUG("output_arp_queue: Entered: ff=%p meta=%p", fins_arp_out, fins_arp_out->ctrlFrame.metaData);
	} else if (fins_arp_out->dataOrCtrl == DATA) {
		PRINT_DEBUG("output_arp_queue: Entered: ff=%p meta=%p", fins_arp_out, fins_arp_out->dataFrame.metaData);
	} else {
		PRINT_DEBUG("output_arp_queue: Entered: ff=%p type=%d", fins_arp_out, fins_arp_out->dataOrCtrl);
	}
