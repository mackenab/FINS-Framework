...
//	if(sizeof(buffer) < sizeof(itoa(ff->dataOrCtrl) + ff->destinationID.id + ff->ctrlFrame.name + itoa(ff->ctrlFrame.opcode) + itoa(ff->ctrlFrame.senderID) + itoa(ff->ctrlFrame.serialNum) + sizeof((char)ff->ctrlFrame.data)))
	...
	//PRINT_DEBUG("temp_buffer: %s\nsize of temp_buffer: %d",temp_buffer,sizeof(temp_buffer));

	//initialize buffer
	PRINT_DEBUG("calculating buffer size");
        int buf_size = strlen(ff->ctrlFrame.name) + 3*sizeof(unsigned char) + 2*sizeof(int) +  		sizeof(unsigned short int) + 		sizeof(unsigned int);
	PRINT_DEBUG("SIZE OF BUF_SIZE = %d", buf_size);
	
	//buf_size += strlen((char *)ff->ctrlFrame.data); // If data is going to be used this needs to be implemented as strlen is crashing
...
//	strncat((unsigned char *)*buffer, (unsigned char *) (&(htonl(ff->ctrlFrame.opcode))), sizeof(int));
	...
	*buffer += sizeof(unsigned int);
	...
	PRINT_DEBUG("final buffer mem addr: %d", (int) *buffer);
	PRINT_DEBUG("start buffer mem addr: %d", (int) temporary);
	int length = ((int) *buffer) - ((int) temporary);
	*buffer = temporary;
	
	PRINT_DEBUG("Buffer: %s", *buffer);
	PRINT_DEBUG("Final value of buffer:%d finswhisperer", length)
 ...
 */
{
	...
	PRINT_DEBUG("mallocing for deserialized fins frame...");
	struct finsFrame * ff =(struct finsFrame *) malloc(sizeof(struct finsFrame));
	PRINT_DEBUG("mallocing for deserialized fins frame sucessful");
	...
	perror("memcpy:");
	PRINT_DEBUG("dataOrCtrl = %d",(int)ff->dataOrCtrl);
	...
	default:
	PRINT_DEBUG("RETURNING ZERO DYING");
		//return 0;
		break;
	}
