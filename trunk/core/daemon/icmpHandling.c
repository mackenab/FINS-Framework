...
void sendmsg_out_icmp(struct nl_wedge_to_daemon *hdr, u_char *data, uint32_t data_len, uint32_t flags, struct sockaddr_in *addr, int addr_len) {
	...
	PRINT_DEBUG("index=%d, dst=%s/%d (%u)", hdr->sock_index, inet_ntoa(addr->sin_addr), dst_port, addr->sin_addr.s_addr);
 ...
 *
 */
void recvmsg_out_icmp(struct nl_wedge_to_daemon *hdr, int data_len, int flags, uint32_t msg_flags) {
...
void poll_in_icmp(struct daemon_call_list *call_list, struct daemon_call *call) {
...
void daemon_icmp_in_fdf(struct finsFrame *ff, uint32_t host_ip, uint16_t host_port, uint32_t dst_ip, uint16_t dst_port) {
	...
	int sock_index = daemon_sockets_match(dst_port, dst_ip, IPPROTO_ICMP); //TODO change for multicast
	if (sock_index == -1) {
		...
		while (call) {
			if (call->call_type == poll_call) { //handle poll_icmp_out call
				poll_in_icmp(call_list, call);
				break;
			}
			call = call->next;
		}
		...
		while (call) {
			if (call->call_type == recvmsg_call) { //TODO handle recvmsg_icmp call
				recvmsg_in_icmp(call_list, call, ff, host_ip, host_port);
