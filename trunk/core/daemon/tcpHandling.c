		...
		if (daemon_sockets[hdr->sock_index].error_buf > 0) {
			...
			*(int *) pt = ff->ctrlFrame.data_len;
			...
			if (send_wedge(nl_sockfd, msg, msg_len, 0)) {
				...
				//TODO error case isn't worked out fully atm
			}
		...
		if (daemon_sockets[hdr->sock_index].data_buf > 0) {
			...
			*(int *) pt = ff->dataFrame.pduLength;
			...
			if (send_wedge(nl_sockfd, msg, msg_len, 0)) {
				...
				metadata_writeToElement(params_resp, "value", &value, META_TYPE_INT);

				metadata_writeToElement(params_resp, "state", &state, META_TYPE_INT);
				metadata_writeToElement(params_resp, "host_ip", &host_ip, META_TYPE_INT);
				metadata_writeToElement(params_resp, "host_port", &host_port, META_TYPE_INT);
				//if (state > SS_UNCONNECTED) {
				metadata_writeToElement(params_resp, "rem_ip", &rem_ip, META_TYPE_INT);
				metadata_writeToElement(params_resp, "rem_port", &rem_port, META_TYPE_INT);
				//}
		...
		if (daemon_sockets[hdr->sock_index].data_buf > 0) {
			...
			*(int *) pt = ff->dataFrame.pduLength;
			...
			if (send_wedge(nl_sockfd, msg, msg_len, 0)) {
				...
				if (daemon_fcf_to_tcp(params, gen_control_serial_num(), CTRL_SET_PARAM, SET_PARAM_TCP_HOST_WINDOW)) {
					PRINT_DEBUG("Exited, normal: hdr=%p", hdr);
				} else {
					PRINT_ERROR("Exited, fail sending flow msgs: hdr=%p", hdr);
					metadata_destroy(params);
	...
	if (mask & events) { //If were immediate events reply; otherwise, block
		...
		*(uint32_t *) pt = mask;
		...
		if (events & (POLLIN | POLLRDNORM | POLLPRI | POLLRDBAND)) {
			...
			PRINT_ERROR("Unhandled events? events=%x", events);
			nack_send(hdr->call_id, hdr->call_index, hdr->call_type, 0);
		}
	}
}
...
void sendmsg_in_tcp(struct finsFrame *ff, uint32_t call_id, int call_index, uint32_t call_type, uint64_t sock_id, int sock_index, uint32_t flags) { //TODO remove data? not needed
	...
	int ret = 0;
	ret += metadata_readFromElement(params, "ret_msg", &ret_msg) == CONFIG_FALSE;
...
void poll_in_tcp_fcf(struct finsFrame *ff, uint32_t call_id, int call_index, uint32_t call_type, uint64_t sock_id, int sock_index, uint32_t data) {
	...
	int ret = 0;
	ret += metadata_readFromElement(params, "ret_msg", &ret_msg) == CONFIG_FALSE;
	//ret += metadata_readFromElement(params, "mask", &mask) == CONFIG_FALSE;
...
void recvmsg_in_tcp_fdf(struct daemon_call_list *call_list, struct daemon_call *call, struct finsFrame *ff, uint32_t addr_ip, uint16_t addr_port,
		uint32_t flags) {
	PRINT_DEBUG("Entered: ff=%p, call=%p, call_id=%u, addr=%u/%u, flags=%u", ff, call, call->call_id, addr_ip, addr_port, flags);
	...
	*(int *) pt = ff->dataFrame.pduLength;
	...
	PRINT_DEBUG("msg_len=%d msg=%s", msg_len, msg);
	if (send_wedge(nl_sockfd, msg, msg_len, 0)) {
		PRINT_ERROR("Exited: send_wedge error: ff=%p, call=%p", ff, call);
		nack_send(call->call_id, call->call_index, call->call_type, 0);
	} else {

		daemon_sockets[call->sock_index].data_buf -= ff->dataFrame.pduLength;
...
void daemon_tcp_in_fdf(struct finsFrame *ff, uint32_t src_ip, uint32_t dst_ip) {
	...
	int sock_index = daemon_sockets_match_connection(src_ip, src_port, dst_ip, dst_port, IPPROTO_TCP);
	if (sock_index == -1) {
		sock_index = daemon_sockets_match_connection(src_ip, src_port, 0, 0, IPPROTO_TCP);
	}
	if (sock_index == -1) {
		PRINT_ERROR("No match, freeing ff=%p, src=%u/%u, dst=%u/%u", ff, src_ip, src_port, dst_ip, dst_port);
		/*#*/PRINT_DEBUG("post@@@@@@@@@@@@@@@@@@@@@@@");
		sem_post(&daemon_sockets_sem);
		...
		while (call) {
			if (call->call_type == recvmsg_call && !(call->flags & (MSG_ERRQUEUE))) { //signal first recvmsg for data
				//recvmsg_in_tcp(call_list, call, params, ff->dataFrame.pdu, ff->dataFrame.pduLength, src_ip, src_port, 0);
				return;
			}
			call = call->next;
		}
...
void daemon_tcp_in_fdf_old(struct finsFrame *ff, uint32_t src_ip, uint32_t dst_ip) {
	PRINT_DEBUG("Entered: ff=%p src_ip=%u, dst_ip=%u", ff, src_ip, dst_ip);
	...
	int ret = 0;
	ret += metadata_readFromElement(params, "src_port", &src_port) == CONFIG_FALSE; //ICMP doesn't have
	ret += metadata_readFromElement(params, "dst_port", &dst_port) == CONFIG_FALSE;
	...
	if (sock_index == -1) {
		...
		while (call) {
			if (call->call_type == recvmsg_call) {
				recvmsg_in_tcp_fdf(call_list, call, ff, src_ip, src_port, 0);
...
void daemon_tcp_in_error(struct finsFrame *ff, uint32_t src_ip, uint32_t dst_ip) {
	...
	if (sock_index == -1) {
		PRINT_ERROR("No match, freeing ff");
		/*#*/PRINT_DEBUG("post@@@@@@@@@@@@@@@@@@@@@@@");
		sem_post(&daemon_sockets_sem);
		...
		if (write_queue(ff, daemon_sockets[sock_index].error_queue)) {
			daemon_sockets[sock_index].error_queue++;
