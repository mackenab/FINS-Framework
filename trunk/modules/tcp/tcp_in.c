		...
		if (seg->ack_num == conn->send_seq_num) {
			...
			if (!updated) {
				...
				if (md->fast_enabled && conn->duplicate >= md->fast_duplicates) {
					...
					if (conn->fast_ack == conn->send_seq_num) {
						//conn->fast_retries++;
					} else {
						conn->fast_retries = 0;
					}

					if (conn->fast_retries <= md->fast_retries) {
						//RTT
						conn->rtt_flag = 0;
						...
						case RENO_AVOIDANCE:
							if (conn->send_seq_num == conn->issn) {
								...
								conn->fast_flag = 1;
								conn->fast_ack = conn->send_seq_num;
			...
			if (conn->rtt_flag && seg->ack_num == conn->rtt_seq_end) {
				...
				//PRINT_INFO("dev=%f, est=%f, timeout=%f",conn->stats.rtt_dev_total/conn->stats.rtt_dev_count, conn->stats.rtt_est_total/conn->stats.rtt_est_count, conn->stats.timeout_total /conn->stats.timeout_count);
			}
...
int process_flags(struct tcp_conn *conn, struct tcp_seg *seg, uint16_t *send_flags) {
	...
	case TS_ESTABLISHED:
		...
		if (seg->flags & (FLAG_SYN)) {
			...
			tcp_conn_send_fcf(conn, CTRL_ALERT, TCP_ALERT_SHUTDOWN, FCF_FALSE, TCP_STATUS_RD);
...
void tcp_syn(struct fins_module *module, struct tcp_conn_stub *conn_stub, struct tcp_seg *seg) {
	PRINT_DEBUG("Entered: conn_stub=%p, seg=%p", conn_stub, seg);
	struct tcp_data *md = (struct tcp_data *) module->data;
	...
	conn_stub->threads--;
	PRINT_DEBUG("leaving thread: conn_stub=%p, threads=%d", conn_stub, conn_stub->threads);PRINT_DEBUG("conn_stub_list post***************");
	...
	if (seg->flags & FLAG_RST) {
		...
		if (seg->flags & FLAG_ACK) {
			if (seg->ack_num == conn->send_seq_num + 1) {
				...
				if (conn->ff) {
					module_reply_fcf(module, conn->ff, FCF_TRUE, 0);
					conn->ff = NULL;
				} else {
					PRINT_WARN("todo error");
				}
...
void tcp_recv(struct fins_module *module, struct tcp_conn *conn, struct tcp_seg *seg) {
	...
	conn->threads--;
	PRINT_DEBUG("leaving thread: conn=%p, threads=%d", conn, conn->threads);PRINT_DEBUG("conn_list post***************");
