...
struct arp_request *arp_request_create(struct finsFrame *ff, uint64_t src_mac, uint32_t src_ip) {
	PRINT_DEBUG("Entered: ff=%p, mac=0x%012llx, ip=%u", ff, src_mac, src_ip);
	...
	PRINT_DEBUG("Exited: ff=%p, mac=0x%012llx, ip=%u, request=%p", ff, src_mac, src_ip, request);
	return request;
}
