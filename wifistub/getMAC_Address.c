...
void getMAC_Address(unsigned char macAddress[], char *EtherDevice)
...
TargetDevice = (char *)malloc (100);
strcat(TargetDevice,EtherDevice);
strcat(TargetDevice,"     Link encap:Ethernet  HWaddr ");

while(!feof(fs2))
{
	fgets(buffering,100,fs2);
	macat=strstr(buffering,TargetDevice);
