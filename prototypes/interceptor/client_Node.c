...
int main()
{
	...
	server_addr2.sin_addr.s_addr = xxx(192,168,1,130);
	server_addr3.sin_addr.s_addr = xxx(192,168,1,131);
	   ...
	   while ( i < 100)
	   {
		...
		/*
			...
			//	if (i % 100 ==0 )
			      ...
			      numbytes1= sendto(sock1, send_data, strlen(send_data) , 0,
				      (struct sockaddr *)&server_addr1, sizeof(struct sockaddr));

				 numbytes2= sendto(sock2, send_data, strlen(send_data) , 0,
				      (struct sockaddr *)&server_addr2, sizeof(struct sockaddr));

				 numbytes3= sendto(sock3, send_data, strlen(send_data) , 0,
