...
{
	PRINT_DEBUG("IP4_in has been just called");
	stats.receivedtotal++;
	...
	{
		stats.badlen++;
		PRINT_ERROR("The declared length is not equal to the actual length.");
		if (header.packet_length > len)
		{
			stats.droppedtotal++;
			//free ppacket
	...
	{
		PRINT_DEBUG("Will  be sent to IPv4_forward");
		if(IP4_forward(ppacket, header.destination, len)){
			PRINT_DEBUG("IPv4_forward success ");
			return;
		}
		stats.droppedtotal++;
		PRINT_DEBUG("PACKET DROPPED, PATH TO DESTINATION DOES NOT EXIST");
	...
	{
		stats.delivered++;
		PRINT_DEBUG("packet is not fragmented");
