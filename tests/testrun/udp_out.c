...
void udp_out(struct finsFrame* ff) {
	...
	ff->dataFrame.pdu=&packet;											/* copies the UDP packet into the memory that has been allocated for the PDU */
	...
	udpStat.totalSent++;
	 ...
	 IP4_out(&packet, ntohs(packet.u_len), (IP4addr)(meta->u_IPsrc), (IP4addr)(meta->u_IPdst),
			 IP4_PT_UDP);
	//sendToSwitch(newFDF);
}
