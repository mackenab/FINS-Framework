...
struct tableRecord* UpdateCache(struct finsFrame QueryReply, struct tableRecord *PTRX)
	...
	FreeRecords(QueryReply.ctrlFrame.replyRecord);  /*free memory which had been allocated for the initial searching*/
...
struct destinationList * SearchLocalTable(struct tableRecord *ptrCache, unsigned char vc, unsigned char DF)
	...
	if (L==0) {free(First); /**if no match was found or empty cache then the memory has to be freed*/
	printf(" None Found\n");
   fflush(stdout);
...
void PrintDestinationList (struct destinationList *dList)
{
	...
	while (dList!=NULL){
		...
		printf("VCI-based, Direction Flag-based Destination ID : %d \n",dList->id);
		fflush(stdout);
